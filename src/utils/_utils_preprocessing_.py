...
class NILMData(object):
    def __init__(self,
        ...
        self.house_test_indicies = test_house_indices
    ...
    def create_downsampled_dataset(self, indices, appliance_names, downsampling_func, threshold=0.05):
        data = {}
    ...
    def BuildSaveNILMDataset(self, perc=1, mask_index='Time', downsampling_func=None, threshold=None):
        ...
        training_downsampled_datasets = None
        testing_downsampled_datasets = None

        #if self.house_train_indicies is not None and downsampling_func is not None:
        ...
        if self.house_test_indicies is not None:
            test = self.GetNILMDataset(self.house_test_indicies, perc=perc, mask_index=mask_index, downsampled=testing_downsampled_datasets)
            if self.save_path is not None:
    ...
    def GetNILMDataset(self, house_indices, perc=1, mask_index='Time', downsampled=None):
        """
        ...
        for indice in house_indices:
            ...
            X = np.empty((len(house_indices) * n_wins, len(self.appliance_names), 2, self.window_size))
...
class REFITData(NILMData):
    def __init__(self,
                 ...
                 window_size,
                 limit_ffill,
                 data_path=os.getcwd()+'/data/REFIT/RAW_DATA_CLEAN/',
                 cutoff=6000,
                 window_stride=None,
                 save_path=None,
                 test_house_indices=None):
        super().__init__(data_path=data_path,
                         ...
                         test_house_indices=test_house_indices,
...
class UKDALEData(NILMData):
    def __init__(self,
                 ...
                 window_size,
                 limit_ffill,
                 data_path=os.getcwd()+'/data/UKDALE/',
                 cutoff=6000,
                 window_stride=None,
                 save_path=None,
                 test_house_indices=None,
    ...
    def _get_dataframe(self, indice):
        ...
        for appliance in self.appliance_names:
            ...
            if indice in self.test_indices:
                house_path = path_house + f'channel_1_{appliance}.dat'
            ...
            if len(house_label.loc[house_label['appliance_name']==appliance]['id'].values) != 0:
                ...
                if indice in self.test_indices:
                    appl_path = path_house+'channel_'+str(i)+f'.dat'
